...
namespace Akamai\Config;

use Akamai\Exception\EdgeAuthParameterException as ParameterException;

class EdgeAuthConfigPD {
    ...
    public function set_algo($algo) {
        if (in_array($algo, array('sha256','sha1','md5'))) {
            ...
            throw new ParameterException("Invalid algorithme, must be one of 'sha256', 'sha1' or 'md5'");
        }
    }
